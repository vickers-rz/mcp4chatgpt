"""轻量本地知识库：导入、切块、检索与学习辅助。

该模块没有引入向量数据库，而是把来源及文本块保存为 JSON，并用简单 token 集合
进行词项重合评分。它展示了 MCP 的一个重要特点：协议只规定“如何暴露工具”，并不
限定工具内部必须使用哪种检索技术。后续可把这里替换为 embedding/SQLite/向量库，
而无需改变客户端的 ``tools/list`` 与 ``tools/call`` 交互方式。

文件导入前仍要经过允许目录校验；持久化采用临时文件替换，损坏存储会被隔离，避免
一次异常写入导致整个服务无法启动。
"""
